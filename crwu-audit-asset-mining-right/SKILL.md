---
name: crwu-audit-asset-mining-right
description: Use when crwu-audit has selected the canonical 矿业权 asset label for professional asset-object review.
---
...
仅经 `crwu-audit` router 编排调用，禁止单独调用。本技能只审核 canonical `矿业权` 对象的资产专业事项，不查询报告记录、不准备材料、不重新下载规则，也不替代其他轴技能。
...
- 同一份 `route_profile`，且 `asset_types[]` 已命中 canonical `矿业权`；
...
4. 输出对象专业 findings；每条均带 `source_skills[]`，其中至少包含 `crwu-audit-asset-mining-right`。相同事实由其他技能共同支持时保留全部来源，冲突时并列交人工复核。
