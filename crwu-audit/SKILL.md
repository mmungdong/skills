...
10. **汇总输出**：意见按 报告/评估说明/测算明细表 分区去重；附路由路径、适用规则集快照、
   route_profile 摘要与三段式引用（KB 06/07，禁止放宽）；
11. **统计回填**：route 命中/降级/空能力/weak_structured 计数 → KB 08 台账。
