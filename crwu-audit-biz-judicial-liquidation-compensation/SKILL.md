---
name: crwu-audit-biz-judicial-liquidation-compensation
description: Use when crwu-audit has selected the canonical 司法清算与补偿 business label for scenario-specific review.
---
...
- 同一份 `route_profile`，且 `business_types[]` 已命中 canonical `司法清算与补偿`；
...
5. 输出业务专业 findings；每条带 `source_skills[]`（至少含 `crwu-audit-biz-judicial-liquidation-compensation`）与本次下载证据（文件、行号、库内路径、运行时 `nodeId`、`exportedAt`）。
