---
name: crwu-audit-biz-consulting-review
description: Use when crwu-audit has selected the canonical 咨询复核与其他 business label for scenario-specific review.
---
...
仅经 `crwu-audit` router 编排调用，禁止单独调用。本技能只审一级业务 `咨询复核与其他` 的场景规则（评估目的、经济行为、场景对口径/材料/判断的影响），不查询报告记录、不重新获取材料、不重新下载规则正文。
...
- 同一份 `route_profile`，且 `business_types[]` 已命中 canonical `咨询复核与其他`；
...
5. 输出业务专业 findings；每条带 `source_skills[]`（至少含 `crwu-audit-biz-consulting-review`）与本次下载证据（文件、行号、库内路径、运行时 `nodeId`、`exportedAt`）。
