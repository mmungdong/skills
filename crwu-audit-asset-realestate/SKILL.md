...
description: Use when crwu-audit has selected the canonical 房地产（房建类）asset label for professional asset-object review.
---
...
仅经 `crwu-audit` router 编排调用，禁止单独调用。本技能只审核 canonical `房地产` 对象的资产专业事项，不查询报告记录、不准备材料、不重新下载规则，也不替代其他轴技能。
...
不得决定租赁、清算、资产处置、拍卖、抵押质押、财务报告等业务标签或业务判断；这些事项由 router 同时加载的 business skills 处理。材料含测算、明细或汇总表时，表格勾稽由 router 并入 public skill 执行，本技能只提供房地产对象侧的界面与一致性检查。

## 输入

- 同一份 `route_profile`，且 `asset_types[]` 已命中 canonical `房地产`；
...
3. [02-review-focus.md](references/02-review-focus.md)：按房地产对象专业关注点实施检查。
...
4. 输出对象专业 findings；每条均带 `source_skills[]`，其中至少包含 `crwu-audit-asset-realestate`。相同事实由其他技能共同支持时保留全部来源，冲突时并列交人工复核。
